...
class AsyncPocketOptionClient:
    ...
    def __init__(
        ...
        Args:
            ...
            proxy: Proxy URL(s) - can be a single string or list of strings for rotation
                   Format: socks5://user:pass@host:port or http://user:pass@host:port
                   Examples:
                   - Single: "socks5://127.0.0.1:1080"
                   - Multiple: ["socks5://proxy1.com:1080", "socks5://proxy2.com:1080"]
            proxy_rotation: Enable automatic proxy rotation (default: True)
                           When True and multiple proxies provided, rotates on each connection
        """
        self.raw_ssid = ssid
        ...
        # Handle proxy - convert single to list, normalize
        if proxy:
        ...
        # Core components - proxy will be set on connection via _select_next_proxy()
        ...
        self._server_order_id_to_request_id: Dict[str, str] = {}  # Map server order ID to request ID
        self._candles_cache: Dict[str, List[Candle]] = {}
        self._server_time: Optional[ServerTime] = None
        self._payouts: Dict[str, Dict[str, Any]] = {}  # Store payouts by asset symbol
    ...
    async def _start_regular_connection(
        ...
        for region in regions:
            try:
                ...
                # Select proxy for this connection attempt (rotation)
                if self.proxies:
                    self._select_next_proxy()
    ...
    async def _start_persistent_connection(
        ...
        self._keep_alive_manager.add_event_handler(
            "payout_update", self._on_payout_update
        )
    ...
    def get_payouts(self, symbol: Optional[str] = None) -> Union[Dict[str, Dict[str, Any]], Dict[str, Any], None]:
        """
        Get payout information for assets

        Args:
            symbol: Optional asset symbol (e.g., "#AAPL"). If None, returns all payouts

        Returns:
            If symbol is provided: Dict with payout info for that asset or None if not found
            If symbol is None: Dict of all payouts indexed by symbol
        """
        if not self.is_connected:
            raise ConnectionError("Not connected to PocketOption")
        ...
        if symbol:
            ...
            return self._payouts.copy()

    async def wait_for_payouts(self, timeout: float = 10.0) -> Dict[str, Dict[str, Any]]:
        """
        Wait for payout data to be received from the server

        Args:
            timeout: Maximum time to wait in seconds

        Returns:
            Dict of payouts indexed by symbol
        """
        if not self.is_connected:
            raise ConnectionError("Not connected to PocketOption")
    ...
    def _select_next_proxy(self) -> Optional[str]:
        """Select next proxy from list (with rotation)"""
        if not self.proxies:
            return None
        ...
        if len(self.proxies) == 1:
            ...
            if self._current_proxy_index >= len(self.proxies):
                self._current_proxy_index = 0
            selected = self.proxies[self._current_proxy_index]
            self._current_proxy_index += 1
        else:
            # No rotation, use first proxy
            selected = self.proxies[0]
        
        # Update websocket client with selected proxy
        if selected:
            self._websocket.proxy = selected
            self._websocket._setup_proxy(selected)
        
        if self.enable_logging:
            logger.info(f"Selected proxy: {selected}")
    ...
    async def check_win(
        ...
        Args:
            order_id: Order ID (request_id) to check
        ...
        # First, check if order is already completed
        if order_id in self._order_results:
            result = self._order_results[order_id]
            if self.enable_logging:
                logger.success(
                    f" Order {order_id} already completed - Status: {result.status.value}, Profit: ${result.profit:.2f}"
                )
            return {
                "result": "win"
        ...
        if order_id not in self._active_orders:
            if self.enable_logging:
                logger.warning(
                    f" Order {order_id} not found in active orders or completed results"
                )
            # Still wait a bit in case it's being processed
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if order_id in self._active_orders:
                ...
                if time_remaining <= 0:
                    # Order has expired, wait a bit more for server to send result
                    elapsed = time.time() - start_time
                    if elapsed < max_wait_time:
                        # Log every 5 seconds after expiry
                        if self.enable_logging and (time.time() - last_log_time) >= 5:
                            logger.info(
                                f"⏰ Order {order_id} expired, waiting for server result... ({elapsed:.0f}s elapsed)"
                            )
                            last_log_time = time.time()
                else:
                    # Log progress every 10 seconds
                    if self.enable_logging and (time.time() - last_log_time) >= 10:
                        logger.debug(
                            f"⌛ Order {order_id} still active, expires in {time_remaining:.0f}s"
                        )
                        last_log_time = time.time()
            else:
                # Order not in active orders anymore, check if it's completed
                if order_id in self._order_results:
                    result = self._order_results[order_id]
                    return {
                        "result": "win"
        ...
        if order_id in self._order_results:
            ...
            if self.enable_logging:
                logger.success(
                    f" Order {order_id} completed (final check) - Status: {result.status.value}, Profit: ${result.profit:.2f}"
                )
            return {
                "result": "win"
                if result.status == OrderStatus.WIN
    ...
    async def _on_json_data(self, data: Dict[str, Any]) -> None:
        ...
        if "requestId" in data and "asset" in data and "amount" in data:
            ...
            server_order_id = str(data.get("id", ""))  # Server's order ID if present
            ...
            if (
                ...
                # Store mapping if server order ID is present
                if server_order_id:
                    self._server_order_id_to_request_id[server_order_id] = request_id
                
                if self.enable_logging:
                    logger.success(
                        f" Order {request_id} added to tracking from JSON data"
                        + (f" (server ID: {server_order_id})" if server_order_id else "")
                    )
        ...
        # Check if this is order result data with deals
            ...
            for deal in data["deals"]:
                if isinstance(deal, dict) and "id" in deal:
                    server_order_id = str(deal["id"])
                    
                    # Try to find the request_id for this server order ID
                    request_id = None
                    
                    # First, check if we have a direct mapping
                    if server_order_id in self._server_order_id_to_request_id:
                        request_id = self._server_order_id_to_request_id[server_order_id]
                    # Second, check if deal contains requestId
                    elif "requestId" in deal:
                        request_id = str(deal["requestId"])
                    # Third, try using server_order_id directly (in case they match)
                    elif server_order_id in self._active_orders:
                        request_id = server_order_id
                    # Fourth, search through active orders to find a match by asset/amount/timing
                    else:
                        # Try to find matching order by checking if server_order_id matches any request_id
                        # or by matching deal details
                        for req_id, active_order in self._active_orders.items():
                            # Check if this might be the same order (within reasonable time window)
                            time_diff = abs((datetime.now() - active_order.placed_at).total_seconds())
                            if time_diff < active_order.duration + 10:  # Within order duration + buffer
                                # Check if asset and amount match (best we can do without requestId)
                                deal_amount = float(deal.get("amount", 0))
                                if (active_order.asset == deal.get("asset", "") and 
                                    abs(active_order.amount - deal_amount) < 0.01):
                                    request_id = req_id
                                    # Store the mapping for future reference
                                    self._server_order_id_to_request_id[server_order_id] = request_id
                                    if self.enable_logging:
                                        logger.info(
                                            f" Matched deal {server_order_id} to request_id {request_id} by asset/amount"
                                        )
                                    break

                    if request_id and request_id in self._active_orders:
                        active_order = self._active_orders[request_id]
                        ...
                        # Move from active to completed (use request_id as key)
                        self._order_results[request_id] = result
                        del self._active_orders[request_id]
                        
                        # Clean up mapping
                        if server_order_id in self._server_order_id_to_request_id:
                            del self._server_order_id_to_request_id[server_order_id]

                        if self.enable_logging:
                            logger.success(
                                f" Order {request_id} (server ID: {server_order_id}) completed: {status.value} - Profit: ${profit:.2f}"
                            )
                        await self._emit_event("order_closed", result)
                    elif self.enable_logging:
                        logger.warning(
                            f" Deal received for order ID {server_order_id} but no matching active order found"
                        )
    ...
    async def _on_order_opened(self, data: Dict[str, Any]) -> None:
        ...
        # Try to extract and map server order ID to request ID if present
        if isinstance(data, dict):
            request_id = str(data.get("requestId", ""))
            server_order_id = str(data.get("id", ""))
            
            if request_id and server_order_id and request_id != server_order_id:
                # Store mapping if we have both IDs
                self._server_order_id_to_request_id[server_order_id] = request_id
                if self.enable_logging:
                    logger.debug(
                        f" Mapped server order ID {server_order_id} to request_id {request_id}"
                    )
    ...
    async def _on_payout_update(self, payout_info: Dict[str, Any]) -> None:
        """Handle payout update event"""
        try:
            symbol = payout_info.get("symbol", "")
            if symbol:
                # Store payout info by symbol
                self._payouts[symbol] = payout_info
                if self.enable_logging:
                    logger.info(
                        f"Payout updated for {symbol}: {payout_info.get('payout', 'N/A')}%"
                    )
            await self._emit_event("payout_update", payout_info)
        except Exception as e:
            if self.enable_logging:
                logger.error(f"Error handling payout update: {e}")
