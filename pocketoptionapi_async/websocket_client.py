...
class AsyncWebSocketClient:
    ...
    def __init__(self, proxy: Optional[str] = None):
        ...
        self.proxy = proxy
        self._proxy_obj: Optional[Any] = None
        
        # Parse and setup proxy if provided
        if proxy and PROXY_SUPPORT:
            self._setup_proxy(proxy)
        elif proxy and not PROXY_SUPPORT:
            logger.warning("Proxy specified but python-socks not installed. Proxy will be ignored.")

    def _setup_proxy(self, proxy_url: str) -> None:
        """Setup proxy from URL string"""
        try:
            parsed = urlparse(proxy_url)
            proxy_type = parsed.scheme.lower()
            
            if proxy_type not in ['socks5', 'socks4', 'http']:
                logger.warning(f"Unsupported proxy type: {proxy_type}. Supported: socks5, socks4, http")
                return
            ...
            if proxy_type in ['socks5', 'socks4']:
                if SocksProxy:
                    self._proxy_obj = SocksProxy.from_url(proxy_url)
                else:
                    logger.warning("python-socks not installed. SOCKS proxy not available.")
                    self._proxy_obj = None
            elif proxy_type == 'http':
                # HTTP proxy support via aiohttp
                self._proxy_obj = proxy_url
                
            logger.info(f"Proxy configured: {proxy_type}://{host}:{port}")
    ...
    async def connect(self, urls: List[str], ssid: str) -> bool:
        ...
        for url in urls:
            try:
                ...
                # Connect with proxy if configured
                if self._proxy_obj and PROXY_SUPPORT and SocksProxy:
                    # For SOCKS proxies, we need to create a custom connector
                    if isinstance(self._proxy_obj, SocksProxy):
                        # Parse WebSocket URL to get host and port
                        from urllib.parse import urlparse as parse_url
                        ws_parsed = parse_url(url)
                        ws_host = ws_parsed.hostname
                        ws_port = ws_parsed.port or (443 if ws_parsed.scheme == 'wss' else 80)
                        
                        # Connect through proxy
                        stream = await self._proxy_obj.connect(ws_host, ws_port)
                        try:
                        ...
                        # HTTP proxy - not directly supported by websockets library
                        # Fall back to regular connection
                        logger.warning("HTTP proxy not fully supported for WebSocket, attempting direct connection")
    ...
    async def _handle_payout_data(self, data: List[List[Any]]) -> None:
        """
        Handles parsed payout data from JSON bytes messages.
        
        Args:
            data: List of lists, where each inner list contains payout data.
                  First list may start with 5 (indicator), subsequent lists start with asset ID.
                  Format: [id/symbol, symbol/name, name/type, type/payout, ..., payout, ...]
        """
        try:
            ...
            for asset_data in data:
                ...
                if isinstance(asset_data, list) and len(asset_data) > 5:
                    try:
                        # The structure is: [id, symbol, name, type, ?, payout, ...]
                        # For first list: [5, symbol, name, type, ?, payout, ...]
                        # For other lists: [id, symbol, name, type, ?, payout, ...]
                        ...
                        # If first element is 5 (indicator), we might need to handle it differently
                        # But actually, the data structure seems consistent - index 0 is ID (or 5 for first)
                        payout_info = {
                            "id": asset_id,
                            "symbol": asset_symbol,
                            "name": asset_name,
                        ...
                        logger.warning(
                            f"Payout message element missing for asset_data: {asset_data[:10]}..."
                        )
                    except Exception as e:
                        logger.error(
                            f"Error processing individual asset payout data: {e}"
                        )
                else:
                    logger.debug(
                        f"Skipping invalid payout data format (length: {len(asset_data) if isinstance(asset_data, list) else 'N/A'})"
                    )
        except Exception as e:
            logger.error(
                f"Error in _handle_payout_data: {e}"
            )
    ...
    async def _handle_payout_message(self, message: str) -> None:
        ...
        try:
            ...
            await self._handle_payout_data(data)
        ...
        try:
            ...
            if isinstance(message, bytes):
                ...
                try:
                    ...
                    # Handle payout data (format: [[5, symbol, name, type, ..., payout, ...], ...])
                    # Note: First list starts with 5 as indicator, but all lists contain asset data
                    if isinstance(json_data, list) and len(json_data) > 0:
                        # Check if this is payout data (first element is a list that might start with 5 or contains asset data)
                        if isinstance(json_data[0], list) and len(json_data[0]) > 5:
                            # Check if first element is 5 (payout indicator) or has asset structure (symbol at index 1)
                            first_list = json_data[0]
                            # If first element is 5, or if we have a string at index 1 (likely symbol), it's payout data
                            if (first_list[0] == 5) or (len(first_list) > 1 and isinstance(first_list[1], str) and first_list[1].startswith('#')):
                                # This is payout data - parse it
                                await self._handle_payout_data(json_data)
                                return
                    ...
                    elif isinstance(json_data, dict) and "requestId" in json_data and json_data["requestId"] == "buy":
